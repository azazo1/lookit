...
import { imageSize } from "./image-utils.ts";
...
async function loadImageMetas(paths: string[]): Promise<ImageMeta[]> {
  ...
  for (const path of paths) {
    ...
    metas.push({ path, width: size.width, height: size.height });
  }
  return metas;
}
...
function normalizeRegion(value: unknown, width: number, height: number): RegionAnnotation | null {
  ...
  if ((shape === "rect" && points.length < 2) || (shape === "polygon" && points.length < 3) || (shape === "point" && points.length < 1)) {
    return null;
  }
...
async function main(): Promise<void> {
  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      if (url.pathname.startsWith("/api/image/")) {
        const index = Number(url.pathname.slice("/api/image/".length));
  ...
  if (options.output) {
    writeSubmission(options.output, outcome.data);
    console.log(`注解已保存: ${options.output}`);
  }
  if (options.text) {
    console.log(formatText(outcome.data));
  } else {
